...
from pytorch_tabnet.diffLoss import DiffLoss
...
class global_encoder(torch.nn.Module):
    def __init__(self,input_size):
        ...
        self.output_block = torch.nn.Linear(input_size,128)
    def forward(self,x):
        fc_out = self.basicblock1(x)
        atten1 = self.attention1(fc_out)
        atten1 = self.softmax(atten1)
        fc_out = self.basicblock3(fc_out)
        atten1 = atten1 * fc_out
        fc_out = 0.5*fc_out+0.5*atten1
...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        for i in range(1,len(split_set)):
            loss = self.diff_citer(split_set[i-1],split_set[i])
            diff_loss += loss

        M_loss /= self.n_steps
        return steps_output,(M_loss,diff_loss)
...
class TabNetNoEmbeddings(torch.nn.Module):
    def __init__(
        ...
        #new part
        self.global_feat = global_encoder(self.input_dim)
        self.bn1 = BatchNorm1d(self.input_dim,momentum = 0.01)
