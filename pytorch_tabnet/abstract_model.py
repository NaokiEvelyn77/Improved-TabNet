...
class TabModel(BaseEstimator):
    ...
    def _train_epoch(self, train_loader):
        ...
        for batch_idx, (X, y) in enumerate(train_loader):
            ...
            #print diff loss
            diff_loss += batch_logs['diffloss']

        epoch_logs = {"lr": self._optimizer.param_groups[-1]["lr"]}
        self.history.epoch_metrics.update(epoch_logs)
        #print
        print('Diff loss:{}'.format(diff_loss/len(train_loader)))
        return
    ...
    def _train_batch(self, X, y):
        ...
        output,M_diff_loss = self.network(X)

        M_loss,diff_loss = M_diff_loss
        #predict loss
        ...
        batch_logs['diffloss'] = diff_loss.cpu().detach().numpy().item()
